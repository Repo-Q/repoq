...
k-repair selection based on ΔQ impact estimation.
...
class RefactoringTask:
    ...
    def to_markdown(self) -> str:
        ...
        lines = [
            ...
            f"**Priority**: {priority_emoji.get(self.priority, '⚪')} {self.priority.upper()}",
            f"**Expected ΔQ**: +{self.delta_q:.1f} points",
    ...
    def to_github_issue(self, repo_name: str = "repoq") -> dict:
        ...
        body_lines = [
            f"## 📋 Refactoring Task: `{self.file_path}`",
            "",
            f"**Expected quality improvement**: ΔQ = +{self.delta_q:.1f}",
            f"**Estimated effort**: {self.estimated_effort}",
            "",
            "### Current Metrics",
            "",
        ...
        body_lines.extend(
            [
                "",
                "---",
                f"*Generated by RepoQ refactor-plan command*",
            ]
        )

        return {
            "title": f"Refactor: {Path(self.file_path).name} (ΔQ: +{self.delta_q:.1f})",
            "body": "\n".join(body_lines),
            "labels": labels,
        }
...
class RefactoringPlan:
    ...
    def to_markdown(self) -> str:
        ...
        lines = [
            ...
            f"- **Total ΔQ**: +{self.total_delta_q:.1f}",
...
def calculate_delta_q(file_data: dict) -> float:
    """Calculate expected ΔQ if file issues are fixed.

    Args:
        file_data: File metrics dictionary from JSON-LD

    Returns:
        Expected quality score improvement (ΔQ)
...
def generate_recommendations(file_data: dict) -> List[str]:
    ...
    if todos >= 5:
        recommendations.append(
            f"📝 Resolve {todos} TODO comments (prioritize blocking issues)"
        )
    elif todos > 0:
...
def assign_priority(delta_q: float, complexity: float) -> str:
    """Assign priority based on ΔQ and complexity.
...
def generate_refactoring_plan(
    ...
    Algorithm:
        ...
        2. Calculate ΔQ for each file
        3. Filter by min_delta_q threshold
        4. Sort by ΔQ descending (greedy)
