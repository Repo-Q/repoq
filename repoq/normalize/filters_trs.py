...
def _would_improve_pattern_set(
    new_pattern: str, old_patterns: List[str], all_patterns: List[str]
) -> bool:
...
class AdvancedFilterParser:
    ...
    def _preprocess_filter_expression(self, expr: str) -> str:
        ...
        processed = re.sub(r"(\w+)\s*([><=!]+)\s*(\w+)", r"file_prop('\1', '\2', '\3')", processed)
...
def canonicalize_filter_advanced(
    filter_spec: Union[str, Dict[str, Any], FilterExpression], form: str = "dnf"
) -> str:
    """
...
def check_filter_equivalence(
    filter1: Union[str, Dict[str, Any]], filter2: Union[str, Dict[str, Any]]
) -> bool:
    """
...
if __name__ == "__main__":
    ...
    complex_filter = {
        "operator": "and",
        "operands": [
            {"operator": "or", "operands": [{"pattern": "*.py"}, {"pattern": "*.js"}]},
