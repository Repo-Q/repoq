...
class VersionInterval(Term):
    ...
    def is_universal(self) -> bool:
        ...
        return (
            self.min_version == ZERO_VERSION
            and self.include_min
            and self.max_version == MAX_VERSION
            and not self.include_max
        )

    def is_exact(self) -> bool:
        """Check if interval represents exact version."""
        return self.min_version == self.max_version and self.include_min and self.include_max
    ...
    def _overlaps_or_adjacent(self, other: "VersionInterval") -> bool:
        ...
        if self.max_version == other.min_version and (self.include_max or other.include_min):
            return True

        # Case 2: other.max touches self.min
        if other.max_version == self.min_version and (other.include_max or self.include_min):
            return True
