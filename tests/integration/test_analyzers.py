...
class TestHistoryAnalyzerIntegration:
    ...
    def test_history_with_file_churn(self, python_repo: Path):
        ...
        # Some files should have churn data
        files_with_churn = [f for f in project.files.values() if f.code_churn > 0]
        assert len(files_with_churn) > 0
...
class TestHotspotsAnalyzerIntegration:
    ...
    def test_identify_hotspots(self, python_repo: Path):
        ...
        # Verify hotspot scores were calculated
        files_with_hotspots = [f for f in project.files.values() if f.hotspot_score > 0]
        assert len(files_with_hotspots) > 0
    ...
    def test_hotspot_scores_normalized(self, python_repo: Path):
        ...
        # All hotspot scores should be in [0, 1] range
        for f in project.files.values():
            if f.hotspot_score > 0:
                assert 0 <= f.hotspot_score <= 1
...
class TestFullAnalysisPipeline:
    ...
    def test_full_analysis_workflow(self, python_repo: Path, temp_dir: Path):
        ...
        project = Project(
            id=f"file://{python_repo}",
            name=python_repo.name,
            repository_url=None
        )
        config = AnalyzeConfig(mode="full")
    ...
    def test_analysis_with_real_metrics(self, python_repo: Path):
        ...
        total_loc = sum(f.lines for f in project.files.values())
        assert total_loc > 0
