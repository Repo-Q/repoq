...
class TestCLIHistoryE2E:
    ...
    def test_history_analysis(self, python_repo: Path, temp_dir: Path):
        ...
        data = json.loads(output.read_text())
        assert "prov:wasAssociatedWith" in data or "repoq:hasContributor" in data
...
class TestCLIFullWorkflow:
    ...
    def test_full_analysis_workflow(self, python_repo: Path, temp_dir: Path):
        ...
        assert jsonld_data["@type"] == "repoq:Project"
        assert "repoq:hasFile" in jsonld_data
...
def test_self_analysis_e2e(temp_dir: Path):
    ...
    result = subprocess.run(
        [
            ...
            str(repo_root),
            "--output", str(output),
            "--exclude", "site/**,tmp/**,.venv/**,*.jsonld,*.md",
            "--extensions", "py",
            "--max-files", "200",
        ],
        capture_output=True,
