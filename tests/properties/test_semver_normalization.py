...
def simple_range_strategy(draw):
    ...
    range_type = draw(
        st.sampled_from(["exact", "caret", "tilde", "gte", "lte", "gt", "lt", "wildcard"])
    )
...
class TestSemVerNormalizationProperties:
    ...
    def test_union_commutativity(self, range_a, range_b):
        ...
        try:
            ...
            if isinstance(term_a, VersionInterval) and isinstance(term_b, VersionInterval):
                ...
                assert (
                    union_ab_sorted == union_ba_sorted
    ...
    def test_normalize_preserves_semantics(self, range_str):
        ...
        try:
            ...
            for version in test_versions:
                ...
                assert (
                    original_contains == normalized_contains
    ...
    def test_caret_range_semantics(self, range_str):
        ...
        try:
            ...
            if version.major > 0:
                ...
                assert not _term_contains_version(
                    term, upper_bound
    ...
    def test_universal_range(self):
        ...
        test_versions = [Version(0, 0, 0), Version(1, 0, 0), Version(999, 999, 999)]
    ...
    @given(min_version=version_strategy(), max_version=version_strategy())
    @settings(max_examples=30, deadline=3000)
    def test_interval_operations(self, min_version, max_version):
        ...
        try:
            ...
            assert self_intersect == interval, f"Self-intersection should be identity: {interval}"

            # Test union with self
            self_union = interval.union(interval)
            assert self_union == [interval], f"Self-union should be singleton: {interval}"
...
class TestSemVerParsing:
    ...
    def test_parse_exact_version(self):
        ...
        cases = ["1.2.3", "0.0.1", "10.20.30", "1.0.0-alpha", "1.0.0-alpha.1", "1.0.0+build.1"]
    ...
    def test_parse_wildcard_ranges(self):
        ...
        cases = [("*", "*"), ("1.x", "[1.0.0, 2.0.0)"), ("1.2.x", "[1.2.0, 1.3.0)")]
    ...
    def test_parse_compound_ranges(self):
        ...
        cases = [">=1.2.3 <2.0.0", ">=1.0.0 <=1.5.0", ">1.0.0 <1.2.0"]
    ...
    def test_parse_union_ranges(self):
        ...
        cases = ["1.2.3 || 2.0.0", "^1.0.0 || ^2.0.0", "~1.2.0 || >=2.0.0 <3.0.0"]
