...
from repoq.normalize.rdf_trs import (
    ...
    RDFGraph,
    Triple,
    _canonicalize_blank_nodes,
    _compute_blank_node_hash,
    canonicalize_rdf,
    rdf_hash,
)
...
def simple_jsonld_strategy(draw):
    ...
    for _ in range(num_props):
        ...
        prop_value = draw(st.one_of(st.text(min_size=1, max_size=20), st.integers(), st.booleans()))
...
class TestRDFNormalizationProperties:
    ...
    def test_deterministic_output(self, jsonld_obj):
        ...
        try:
            ...
            assert json1 == json2, f"Non-deterministic output for {jsonld_obj}"
        except Exception:
            assume(False)
    ...
    def test_blank_node_canonicalization(self, graph):
        ...
        try:
            ...
            for node in blank_nodes:
                assert node.canonical_id is not None, f"Blank node missing canonical ID: {node}"
                assert node.canonical_id.startswith(
                    "b"
            ...
            assert canonical_graph == canonical_graph2, "Blank node canonicalization not stable"
    ...
    def test_triple_ordering(self, triple):
        ...
        try:
            ...
            assert isinstance(sort_key, tuple), f"Sort key should be tuple, got {type(sort_key)}"
            assert len(sort_key) == 3, f"Sort key should have 3 elements, got {len(sort_key)}"
        except Exception:
            assume(False)
    ...
    def test_triple_sorting_consistency(self, graph):
        ...
        try:
            ...
            for i in range(len(graph.triples) - 1):
                ...
                assert curr_key <= next_key, f"Triples not sorted: {curr_key} > {next_key}"
        except Exception:
            assume(False)
    ...
    def test_property_ordering(self):
        ...
        assert keys == sorted_keys, f"Properties not sorted: {keys} != {sorted_keys}"
...
class TestRDFRewriteSystem:
    ...
    def test_rewrite_system_basic(self):
        ...
        triple = Triple(
            IRI("http://example.com/alice"), IRI("http://example.com/name"), Literal("Alice")
        )
...
class TestJSONLDIntegration:
    ...
    def test_blank_node_jsonld(self):
        ...
        obj = {"@id": "_:b1", "name": "Anonymous", "knows": "_:b2"}
    ...
    def test_nested_objects(self):
        ...
        obj = {"z_property": {"nested_z": "last", "nested_a": "first"}, "a_property": "simple"}
